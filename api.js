...
var getMysqlConnect_Disable = function(){
...
var updateStreamInfo = function(steam_acount, session, area, dak_data, pubg_data){
    //var connection = getMysqlConnect();
...
module.exports = function(router){
            ...
            $('.regions ul li').each(function (idx, element) {
                ...
                if(!$element.hasClass('played_0')){
                    items[curSession].push({
                        title: $element.attr('data-code').toUpperCase().replace('KRJP','KR/JP'),
                        detail: {
                            rank: '',
    ...
    router.route('/getGameSession').get((req, res, next) => {
        ...
        .end(function (err, sres) {
            if (err) {
                console.log('err to get https://pubg.me/player/'+ req.query.username);
                return next(err);
            }
            var $ = cheerio.load(sres.text);
            var items = {};
            ...
            $('.page-container .dropdown-menu-profile-overview .dropdown-item.d-flex').each(function (idx, element) {
                ...
                items[curSession].push({
                    title: $element.find('.region').text().toUpperCase().replace('KRJP','KR/JP'),
                    detail: {
            ...
            var sessionArr = [];
                ...
                $('.mode-section').each(function (idx, element) {
                    ...
                    items[idx]= {
                        ...
                        detail: {
                            rs1: $element.find('.rating').find('.value').text().replace(/,/g,'').replace(/(^\s*)|(\s*$)/g, ""),
                            rs2: $element.find('.desc').find('.rank').text().replace('#','').replace(/,/g,'').replace(/(^\s*)|(\s*$)/g, ""),
                            rs3: $element.find('.kd').find('.value').text().replace(/\n /g,'').replace(/(^\s*)|(\s*$)/g, ""),
                            rs4: $element.find('.winratio').find('.value').text().replace(/\n /g,'').replace(/(^\s*)|(\s*$)/g, ""),
                            rs5: $element.find('.games').find('.value').text().replace(/\n /g,'').replace(/(^\s*)|(\s*$)/g, ""),
                            rs6: $element.find('.top10s').find('.value').text().replace(/\n /g,'').replace(/(^\s*)|(\s*$)/g, ""),
                            rs7: $element.find('.deals').find('.value').text().replace(/\n /g,'').replace(/(^\s*)|(\s*$)/g, ""),
                            rs8: $element.find('.headshots').find('.value').text().replace(/\n /g,'').replace(/(^\s*)|(\s*$)/g, "")
                        }
                    };
                });
                resultJSON = {
            ...
            superagent.get(decodeURIComponent(req.query.apiurl)).end(function (err, sres) {
                ...
                $('.profile-match-overview').each(function (idx, element) {
                    ...
                    items[idx]= {
                        ...
                        detail: {
                            rs1: $element.find('.profile-overview-stat-row > .row .col').eq(1).find('.value').text(),
                            rs2: $element.find('.profile-overview-stat-row > .row .col').eq(0).find('.value').text(),
                            rs3: $element.find('.profile-overview-stat-row > .row .col').eq(2).find('.value').text(),
                            rs4: $element.find('.profile-overview-stat-row > .row .col').eq(3).find('.value').text(),
                            rs5: $element.find('.profile-overview-stat-row-small > .row .col').eq(0).find('.value').text(),
                            rs6: $element.find('.profile-overview-stat-row-small > .row .col').eq(3).find('.value-percentage').text(),
                            rs7: $element.find('.profile-overview-stat-row-small > .row .col').eq(2).find('.value').text(),
                            rs8: $element.find('.profile-overview-stat-row-small > .row .col').eq(1).find('.value').text()
                        }
                    };
                });
                resultJSON = {
    ...
    // query stream acount 积分rs1 胜率rs4   K/Drs3
    router.route('/queryacount').get((req, res, next) =>{
        if(req.query.username){
            var connection = getMysqlConnect();
            connection.connect();
            var queryString = "SELECT steam_acount,session,area,dak_data,pubg_data FROM `player_record` where steam_acount = '"+ req.query.username +"' group by steam_acount,session,area order by session desc,area,id desc";
            connection.query(queryString, function (error, results, fields) {
                if (error) throw error;
                var rsData = {};
                for(var n in results){
                    if(!rsData[results[n].session]) {
                        rsData[results[n].session] = {};
                    }
                    var items = results[n].pubg_data && results[n].pubg_data != 'null' ? results[n].pubg_data : results[n].dak_data;
                    items = decodeURIComponent(items);
                    items = JSON.parse(items);
                    rsData[results[n].session][results[n].area] = items;
                }
